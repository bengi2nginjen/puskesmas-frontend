                    ...
                    <div>
                        <div class="row">
                            ...
                            <div class="col-sm btn-group">
                                <a href="isi-form.php?id=<?php echo $form->id ?>" class="btn btn-primary active" aria-current="page">Isi Form</a>
                        ...
                        <div class="col-xl-12 col-lg-7">
                            <div class="user" id="pertanyaan">
                                <div class="button-flex-container mt-3">
                                    <h6 type="text" class="h6">Deskripsi form:</h6>
                                    <h6 type="text" class="h6" id="form-description"><?php echo $form->description; ?></h6>
                                </div>
                                <div id="question-section">
                                    <?php
                                    foreach ($form->questions as $q) {
                                        // print_r($q);
                                        echo '<div class="card border-left-primary shadow mt-3 question-card">';
                                        echo '<div class="card-body">';
                                        if ($q->question != null) {
                                            echo '<h5 class="h5">' . $q->number . '. ' . $q->question . '</h5>';
                                        } else {
                                            echo '<h5 class="h5">' . $q->number . '. <i>Pertanyaan kosong</i></h5>';
                                        }

                                        if ($q->input[0]->type == "DROPDOWN") {
                                            echo '<select class="form-control form-control-user question-select" name="dropdown-' . $q->id . '" data-questionid="' . $q->id . '">';
                                        }
                                        echo '<span class="divider">';
                                        echo '</span>';
                                        foreach ($q->input as $input) {
                                            if ($input->type == "TEXT") {
                                                echo '<div class="text-xs font-weight-bold text-info text-uppercase mb-1">';
                                                echo '<input type="text" class="form-control form-control-user" placeholder="Jawaban..." data-questionid="' . $q->id . '"></input>';
                                                echo '</div>';
                                            } elseif ($input->type == "LONG_TEXT") {
                                                echo '<textarea type="text" class="form-control form-control-user" placeholder="Jawaban..." data-questionid="' . $q->id . '"></textarea>';
                                            } elseif ($input->type == "DATE") {
                                                echo '<input type="date" class="form-control form-control-user" data-questionid="' . $q->id . '">';
                                            } elseif ($input->type == "RADIO") {
                                                echo '<div class="radio-options mt-1">';
                                                echo '<input class="ml-4 mr-2" type="radio" name="radio-' . $q->id . '"value="' . $input->text . '" data-questionid="' . $q->id . '">' . $input->text . '</input>';
                                                echo '</div>';
                                            } elseif ($input->type == "CHECK") {
                                                echo '<div class="checkbox-options mt-1">';
                                                echo '<input class="ml-4 mr-2" type="checkbox" name="checkbox-' . $q->id . '"value="' . $input->text . '" data-questionid="' . $q->id . '">' . $input->text . '</input>';
                                                echo '</div>';
                                            } elseif ($input->type == "DROPDOWN") {
                                                echo '<option class="ml-4 mr-2" value="' . $input->text . '" data-questionid="' . $q->id . '">' . $input->text . '</option>';
                                            }
                                        }
                                        if ($q->input[0]->type == "DROPDOWN") {
                                            echo '</select>';
                                ...
                                <div class="button-flex-container mt-3">
                                    <button id="submit-form-response" href="#" class="form-control btn btn-primary btn-icon-split">
...
<script>
    $(document).ready(function() {
        ...
        // console.log(form);
        $('#submit-form-response').on('click', function() {
